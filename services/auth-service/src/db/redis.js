...
client.on('ready', () => {
  console.log('Cliente Redis listo y conectado.');
  isReady = true;
});

client.on('error', (err) => {
  console.warn('Alerta de Redis (El servicio podría no estar corriendo):', err.message);
  isReady = false;
});
...
client.connect().catch((err) => {
  console.warn('No se pudo conectar a Redis. El sistema continuará operando usando solo PostgreSQL.', err.message);
});
